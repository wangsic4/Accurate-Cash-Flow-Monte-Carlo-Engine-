...
// C# .NET 9 — PROSPECTUS-ACCURATE — 2012 PRICING — 
// =============================================================================
...
namespace INGGoldenSelect
{
    ...
    public class INGLifePayPlus
    {
        ...
        // Add a field to hold the lock rate, and use it once withdrawal begin
        private double? _lockedMawRate = null;
            ...
            for (int t = 0; t < steps; t++)
            {
                ...
                if (cumWd == 0 && t % 4 == 3)
                {
                    if (year < _a.RollUpYears)
                        ...
                        rollUpBase = _a.InitialPremium * (1 + _a.RollUpRate * (year + 1));
                    highestAV = Math.Max(highestAV, avPreWd);
                    benefitBase = Math.Max(rollUpBase, highestAV);
                }

                // 6. Withdrawal
                double mawRate = (age >= _a.FirstWithdrawalAge)
                    ? (_lockedMawRate ?? (_lockedMawRate = _a.MawRate(age)).Value)  // lock at start
                    : _a.MawRate(age); // value unused before start
